...
class Fire(tf.keras.Model):
    def __init__(self, inplanes, squeeze_planes, expand1x1_planes, expand3x3_planes,name=None):
        ...
        '''
        Super() buultin returns a proxy object (temporary object of the superclass) that allow 
        us to access methods of the base class. 
        
        Below, taking 
        " super(Fire, self).__init__(name='%s/fire'%name) "
        as an example,
        Here, a newly defined class "Fire" is also inherit the methods of the base class, 
        which is "tf.keras.Model".
        This line of code "super(Fire, self).__init__(name='%s/fire'%name)"
        is using the proxy object "super(Fire, self)" to access the methods .__init__() which is 
        in the base class. 
        '''
        super(Fire, self).__init__(name='%s/fire'%name)
