...
def load_image(filename, size=None):
    ...
    if size is not None:
        ...
        img = imresize(img, scale_factor)  # this function has been deprecated from SciPy after 1.2.1
    
        # from an online example, using skimage resize() instead
        #newsize = (orig_shape * scale_factor).astype(int)
        #img  = skimage.transform.resize(img, newsize, order=3)
